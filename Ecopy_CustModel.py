...
class RecognizeCustomForms(object):
    def recognize_custom_forms(self, filepath, email, emailsubject, mailReceivedOn, locationPO, processingType, vendorCode):
        ...
        try:
                ...
                for page_number, this_pg in enumerate(pdf_all_pages):
                    ...
                    for scale_value in range(1, 6, 1):
                        ...
                        if len(cpp_result) > 0:
                            ...
                            print(f"Decoded list (Page {page_number + 1}, Scale {scale_value})")
                            for items in decoded_list:
                                try:
                                    ...
                                    self.store_in_database(data_dict, vendorCode, 'RPA_FI_Invoice_MainData')
            ...
            self.store_in_database(str(ex), vendorCode, 'RPA_FI_Invoice_Exceptions')
            ...
            if dupl.empty:
                ...
                #SendMails.sendMailCustom.main1( filepath, ProgramRemarks, UniqueId,Config.serverFileBackupPath_mysore, location, "SAP Posting issue",processingType,vendorCode,inv_no,inv_dt,str('0'),str('0') )
            ...
            try:
                if ProgramRemarks != "":  # if sap posting issues
                    ...
                    #SendMails.sendMailCustom.main1( filepath, ProgramRemarks, UniqueId,Config.serverFileBackupPath_mysore, location, "SAP Posting issue",processingType,vendorCode,inv_no,inv_dt,str('0'),str('0') )
                        # SendMails.sendMailCustom.main1(filepath, ProgramRemarks, sm, Config.serverFileBackupPath_mysore,
                        #                                location, "SAP Posting issue", processingType, vendorCode, InNo,
                        #                                InDate)
                else:
            ...
            print("Excepton at Ecopy_Custmodel",ex)
...
class RecognizeCustomForms(object):
    ...
    def store_in_database(self, data, vendor_code, table_name):
        server = '10.121.2.220'
        database = 'CORPFINANCE'
        username = 'DIGITALFINANCE'
        password = 'F!n@N(3d!g397*'
        ...
        try:
            if table_name == 'RPA_FI_Invoice_MainData':
                sql_insert = f"INSERT INTO {table_name} (column1, column2, ...) VALUES (?, ?, ...)"
                values = (data['column1'], data['column2'], ...)
                cursor.execute(sql_insert, values)
            elif table_name == 'RPA_FI_Invoice_Exceptions':
                sql_insert = f"INSERT INTO {table_name} (exception_column, vendor_code, ...) VALUES (?, ?, ...)"
                values = (data, vendor_code, ...)
                cursor.execute(sql_insert, values)
...
recognizer = RecognizeCustomForms()
recognizer.recognize_custom_forms(filepath, email, emailsubject, mailReceivedOn, locationPO, processingType, vendorCode)
...
if __name__ == "__main__":
    ...
    filepath = "D:\\invoice\\from\\Invoice-32.pdf"
